...
test('reprocessa pendentes apenas quando a última execução foi retryável, sem reciclar waiting_human ou waiting_external', () => {
  ...
  assert.equal(shouldIncludeInCycle('running', undefined), true)
  assert.equal(shouldIncludeInCycle('running', 'failed'), true)
  assert.equal(shouldIncludeInCycle('running', 'running'), false)
  assert.equal(shouldIncludeInCycle('running', 'completed'), false)
})
