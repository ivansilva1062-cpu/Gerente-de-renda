...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      configured: isLoginPinConfigured() || await hasCredential(),
      pinConfigured: isLoginPinConfigured(),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === 'pin-login') {
      if (!isLoginPinConfigured()) {
        return NextResponse.json({ success: false, error: 'PIN de acesso não configurado.' }, { status: 500 })
      }
      if (!verifyLoginPin(typeof body.pin === 'string' ? body.pin : '')) {
